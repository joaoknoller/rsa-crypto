...
const storage = multer.diskStorage({
    ...
    filename: function (request, file, callback) {
        ...
        const novoNomeArquivo = Math.floor(Math.random() * (999909 - 100000 + 1)) + 100000;
...
app.post('/criptografar', upload.single('arquivo'), (req, res) => {
    try {
        ...
        reader.on('data', async chunk => {
            ...
            const cifrado = await cifrar(data, +process.env.N);
...
app.post('/descriptografar', upload.single('arquivo'), (req, res) => {
    try {
        ...
        reader.on('data', async chunk => {
            ...
            const data = Array.from(string.replace(/\[|\]/g, '').split(','), x => +x);
            const decifrado = await decifrar(data, +process.env.N);
